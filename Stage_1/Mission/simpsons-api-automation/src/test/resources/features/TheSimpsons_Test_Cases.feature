Feature: Exponer listado y detalle de personajes de The Simpsons con paginación
  ...
  # ------------------------------------
  Scenario: CP01 - Consultar listado de personajes paginado, validar campos obligatorios y sus tipos
    When realizo una petición GET al endpoint "/characters"
    Then la respuesta debe devolver un código de estado 200
    And debe tener los siguientes campos con sus tipos correctos:
      | campo   | tipo    |
      | count   | int     |
      | next    | String  |
      | prev    | String  |
  ...
  Scenario: CP03 - Consultar listado de personajes paginado, validar estructura minima de cada personaje del listado
    When realizo una petición GET al endpoint "/characters"
    Then la respuesta debe devolver un código de estado 200
    And cada personaje del listado debe tener los siguientes campos y ser del tipo correcto cuando no sean nulos:
      | campo           | tipo    |
      | id              | int     |
      | age             | int     |
      | birthdate       | String  |
  ...
  Scenario: CP04 - Consultar listado de personajes paginado, validar primera página sin indicar un numéro de página
    ...
    And next debe ser de tipo String y debe contener el valor "page=2"
  ...
  Scenario: CP06 - Consultar listado de personajes paginado, consultar segunda página
    ...
    And prev debe ser de tipo String y debe contener el valor "page=1"
    And next no debe ser null
    And next debe ser de tipo String y debe contener el valor "page=3"
  ...
  Scenario: CP07 - Consultar listado de personajes paginado, consultar penúltima página
    ...
    Then la respuesta debe devolver un código de estado 200
    And prev no debe ser null
    And prev debe ser de tipo String y debe contener el valor "page=58"
    And next no debe ser null
    And next debe ser de tipo String y debe contener el valor "page=60"
  ...
  Scenario: CP08 - Consultar listado de personajes paginado, consultar última página
    ...
    Then la respuesta debe devolver un código de estado 200
    And prev no debe ser null
    And prev debe ser de tipo String y debe contener el valor "page=59"
  ...
  Scenario: CP11 - Consultar listado de personajes paginado, validar orden
    When realizo una petición GET al endpoint "/characters"
    Then la respuesta debe devolver un código de estado 200
    And el campo "results" debe estar ordenado por el campo "id" de forma ascendente
  ...
  Scenario: CP12 - Consultar detalle de un personaje correctamente, validar campos obligatorios
    ...
    Then la respuesta debe devolver un código de estado 200
    And debe contener los siguientes campos y ser del tipo correcto cuando no sean nulos:
      | campo           | tipo    |
      | id              | int     |
      | age             | int     |
      | birthdate       | String  |
